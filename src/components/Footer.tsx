...
const FacebookIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-2.21c0-.837.398-1.397 1.583-1.397h2.417v-4.414c-.418-.056-1.854-.189-3.526-.189-3.492 0-5.874 2.138-5.874 6.041v3.169z"/></svg>
);

const InstagramIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 1.366.062 2.633.332 3.608 1.308.975.975 1.245 2.242 1.308 3.608.058 1.266.07 1.646.07 4.85s-.012 3.584-.07 4.85c-.062 1.366-.332 2.633-1.308 3.608-.975.975-2.242 1.245-3.608 1.308-1.266.058-1.646.07-4.85.07s-3.584-.012-4.85-.07c-1.366-.062-2.633-.332-3.608-1.308-.975-.975-1.245-2.242-1.308-3.608-.058-1.266-.07-1.646-.07-4.85s.012-3.584.07-4.85c.062-1.366.332-2.633 1.308-3.608.975-.975 2.242-1.245 3.608-1.308 1.266-.058 1.646-.07 4.85-.07zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948s.014 3.667.072 4.947c.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072s3.667-.014 4.947-.072c4.358-.2 6.78-2.618 6.98-6.98.058-1.281.072-1.689.072-4.948s-.014-3.667-.072-4.947c-.2-4.358-2.618-6.78-6.98-6.98-1.28-.059-1.688-.073-4.947-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c.796 0 1.441.645 1.441 1.44s-.645 1.44-1.441 1.44c-.795 0-1.439-.645-1.439-1.44s.644-1.44 1.439-1.44z"/></svg>
);
      ...
      <div className={`container ${styles.grid}`}>
        <div className={styles.col}>
          <div className={styles.logo}>
            <span style={{ color: "var(--accent-red)", fontWeight: 900, fontSize: "1.5rem", fontStyle: "italic", fontFamily: "var(--font-racing)" }}>BIG BOY</span>
            <span style={{ color: "white", fontWeight: 900, fontSize: "1.5rem", fontStyle: "italic", fontFamily: "var(--font-racing)", display: "block", marginTop: "-5px" }}>ZAMBEZI</span>
          </div>
          <p style={{ marginTop: "1.5rem" }}>
            Authorised Big Boy motorcycle dealer in Sinoville, Pretoria. Sales, service, parts, tuning and custom work.
          </p>
        ...
        <div className={styles.col}>
          ...
          <Link href="/bikes/commercial">Commercial</Link>
          <Link href="/bikes/quads">Quads</Link>
        </div>

        <div className={styles.col}>
          <h3>Workshop</h3>
          <Link href="/services">Services</Link>
          <Link href="/services">Bike Tuning</Link>
          <Link href="/services">Performance Upgrades</Link>
          <Link href="/services">Restoration</Link>
        ...
        <div className={styles.col}>
          ...
          <p style={{ marginTop: "1rem" }}>
            <a href={waLink()} target="_blank" rel="noopener noreferrer" style={{ color: "#25D366", fontWeight: 700 }}>WhatsApp Us</a>
          </p>
        </div>
      </div>

      <div className={styles.bottom}>
        <p>&copy; {year} Big Boy Zambezi. All rights reserved.</p>
      </div>
    </footer>
  );
}
